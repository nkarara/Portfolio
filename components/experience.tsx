      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="space-y-16">
          ...
          <div className="max-w-2xl">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-[-0.03em] text-white mt-2">{t.experience.title}</h2>
            <p className="text-base text-zinc-500 mt-4 leading-relaxed">{t.experience.description}</p>
          </div>

          {/* Timeline */}
          <div className="relative max-w-3xl ml-0 lg:ml-8">
            {/* Vertical line */}
            <div className="absolute left-[7px] top-2 bottom-2 w-px bg-gradient-to-b from-blue-500/30 via-white/6 to-transparent" />

            <div className="space-y-12">
              {items.map((item, index) => (
                <div key={index} className="relative pl-10 group">
                  {/* Timeline dot */}
                  <div className="absolute left-0 top-2 w-[15px] h-[15px] rounded-full border-2 border-zinc-800 bg-zinc-950 flex items-center justify-center transition-all duration-400 group-hover:border-blue-500">
                    <div className="w-[5px] h-[5px] rounded-full bg-zinc-700 transition-colors duration-400 group-hover:bg-blue-500" />
                  </div>
                  ...
                  <div className="vercel-card p-6 sm:p-8 space-y-5">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      ...
                      <span className="text-xs font-mono font-medium px-3 py-1.5 bg-white/3 border border-white/6 text-zinc-500 rounded-lg w-fit whitespace-nowrap">
                        {item.date}
                      </span>
                    </div>
