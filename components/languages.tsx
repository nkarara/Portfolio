...
export function Languages() {
  ...
  const items = [
    { name: t.languages.arabic.name, level: t.languages.arabic.level },
    { name: t.languages.french.name, level: t.languages.french.level },
    { name: t.languages.english.name, level: t.languages.english.level },
  ]

  return (
    <section id="languages" className="py-32 px-4 sm:px-6 lg:px-8 fade-in-view sky-section" ref={ref}>
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="space-y-16">
          ...
          <div className="max-w-2xl">
            ...
            <p className="text-base text-muted-foreground mt-4 leading-relaxed">{t.languages.description}</p>
          </div>
            ...
            {items.map((item, index) => (
              <div key={index} className="vercel-card p-7 space-y-4 group">
                <div className="p-3 bg-blue-500/8 border border-blue-500/10 rounded-xl w-fit transition-colors duration-300 group-hover:bg-blue-500/12">
                  <LanguagesIcon className="h-5 w-5 text-blue-600 dark:text-blue-400" aria-hidden="true" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="font-semibold text-[0.9375rem] text-foreground tracking-[-0.01em]">
                    {item.name}
                  </h3>
                  <p className="text-sm text-muted-foreground leading-relaxed">{item.level}</p>
                </div>
              </div>
            ))}
          </div>
